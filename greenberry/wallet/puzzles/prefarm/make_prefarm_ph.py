...
from greenberry.util.condition_tools import parse_sexp_to_conditions
...
def make_puzzle(amount: int) -> int:
    ...
    if error:
        ...
        for cvp in result_human:
            ...
            total_greenberry += int_from_bytes(cvp.vars[1])
            print(
                f"{ConditionOpcode(cvp.opcode).name}: {encode_puzzle_hash(cvp.vars[0], prefix)},"
                f" amount: {int_from_bytes(cvp.vars[1])}"
            )
    return total_greenberry


total_greenberry = 0
print("Pool address: ")
total_greenberry += make_puzzle(pool_amounts)
print("\nFarmer address: ")
total_greenberry += make_puzzle(farmer_amounts)

assert total_greenberry == calculate_base_farmer_reward(uint32(0)) + calculate_pool_reward(uint32(0))
