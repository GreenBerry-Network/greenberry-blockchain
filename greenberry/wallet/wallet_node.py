...
from greenberry.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from greenberry.protocols.protocol_message_types import ProtocolMessageTypes
from greenberry.protocols.wallet_protocol import (
...
from greenberry.server.node_discovery import WalletPeers
...
from greenberry.util.errors import Err, ValidationError
...
from greenberry.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from greenberry.util.path import mkdir, path_from_root
from greenberry.wallet.block_record import HeaderBlockRecord
from greenberry.wallet.derivation_record import DerivationRecord
from greenberry.wallet.settings.settings_objects import BackupInitialized
from greenberry.wallet.transaction_record import TransactionRecord
from greenberry.wallet.util.backup_utils import open_backup_file
from greenberry.wallet.util.wallet_types import WalletType
from greenberry.wallet.wallet_action import WalletAction
from greenberry.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    server: Optional[GreenBerryServer]
    log: logging.Logger
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]):
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'greenberry keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSGreenBerryConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSGreenBerryConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSGreenBerryConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSGreenBerryConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSGreenBerryConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSGreenBerryConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
