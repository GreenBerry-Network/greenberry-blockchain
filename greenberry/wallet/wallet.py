...
from greenberry.types.announcement import Announcement
...
from greenberry.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
    solution_for_conditions,
)
from greenberry.wallet.puzzles.puzzle_utils import (
...
from greenberry.wallet.secret_key_store import SecretKeyStore
from greenberry.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_greenberry(self, greenberry_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if greenberry_amount < 0:
            utxos = await self.select_coins(abs(greenberry_amount), exclude)
        else:
        ...
        greenberry_amount = spend_value + greenberry_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": greenberry_amount}]
