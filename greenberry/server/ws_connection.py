...
from greenberry.cmds.init_funcs import greenberry_full_version_str
from greenberry.protocols.protocol_message_types import ProtocolMessageTypes
from greenberry.protocols.shared_protocol import Capability, Handshake
from greenberry.server.outbound_message import Message, NodeType, make_msg
from greenberry.server.rate_limits import RateLimiter
...
from greenberry.util.ints import uint8, uint16

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from greenberry.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSGreenBerryConnection:
    """
    ...
    async def perform_handshake(self, network_id: str, protocol_version: str, server_port: int, local_type: NodeType):
        if self.is_outbound:
            outbound_handshake = make_msg(
                ...
                Handshake(
                    'greenberry-' + network_id,
                    protocol_version,
                    greenberry_full_version_str(),
                    uint16(server_port),
            ...
            if ProtocolMessageTypes(inbound_handshake_msg.type) != ProtocolMessageTypes.handshake:
                raise ProtocolError(Err.INVALID_HANDSHAKE)
            if inbound_handshake.network_id != 'greenberry-' + network_id:
