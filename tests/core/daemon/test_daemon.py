...
from greenberry.server.server import ssl_context_for_server
from greenberry.types.peer_info import PeerInfo
from greenberry.util.block_tools import BlockTools
from greenberry.util.ints import uint16
from greenberry.util.ws_message import create_payload
...
class TestDaemon:
    ...
    async def test_daemon_simulation(self, simulation, get_daemon):
        ...
        data = {}
        payload = create_payload("get_blockchain_state", data, service_name, "greenberry_full_node")
        await ws.send_str(payload)
