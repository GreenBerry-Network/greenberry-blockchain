...
from greenberry import __version__
from greenberry.consensus.coinbase import create_puzzlehash_for_pk
from greenberry.ssl.create_ssl import generate_ca_signed_cert, get_greenberry_ca_crt_key, make_ca_cert
from greenberry.util.bech32m import encode_puzzle_hash
from greenberry.util.config import (
    create_default_greenberry_config,
    initial_config_file,
    load_config,
    save_config,
...
from greenberry.util.path import mkdir
from greenberry.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'greenberry keys generate'")
        return None
...
def create_all_ssl(root: Path):
    ...
    greenberry_ca_crt, greenberry_ca_key = get_greenberry_ca_crt_key()
    greenberry_ca_crt_path = ca_dir / "greenberry_ca.crt"
    greenberry_ca_key_path = ca_dir / "greenberry_ca.key"
    greenberry_ca_crt_path.write_bytes(greenberry_ca_crt)
    greenberry_ca_key_path.write_bytes(greenberry_ca_key)
    ...
    greenberry_ca_crt, greenberry_ca_key = get_greenberry_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, greenberry_ca_crt, greenberry_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            print("** Please run `greenberry init` to migrate or create new config files **")
    else:
        return greenberry_init(root_path)


def greenberry_version_number() -> Tuple[str, str, str, str]:
...
def greenberry_minor_release_number():
    res = int(greenberry_version_number()[2])
    print(f"Install release number: {res}")
    return res


def greenberry_full_version_str() -> str:
    major, minor, patch, dev = greenberry_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def greenberry_init(root_path: Path):
    if os.environ.get("GREENBERRY_ROOT", None) is not None:
        print(
            f"warning, your GREENBERRY_ROOT is set to {os.environ['GREENBERRY_ROOT']}. "
            f"Please unset the environment variable and run greenberry init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"GreenBerry directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if GREENBERRY_ROOT is set, or if user has run greenberry init twice
        # before a new update.
        check_keys(root_path)
        print(f"{root_path} already exists, no migration action taken")
        return -1

    create_default_greenberry_config(root_path)
    create_all_ssl(root_path)
    check_keys(root_path)
    print("")
    print("To see your keys, run 'greenberry keys show --show-mnemonic-seed'")
